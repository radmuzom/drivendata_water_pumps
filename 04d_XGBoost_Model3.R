...
set.seed(750)
actual_rounds[55] # Modify for actual rounds used for best iteration
param_list <- list(
  "booster" = "gbtree",
  "verbosity" = 3,
  "validate_parameters" = TRUE,
...
bst <- xgb.train(params = param_list, data = train_train_mat, nrounds = 50)
...
sf <- merge(sf, final_test_pred[, c("id", "status_group")],
            by = "id", sort = FALSE)
fwrite(sf, "Submissions/Submission_20200408.csv")
